...
void mouseDragged() {
  if (Map3D.active) {
    if (mouseButton == LEFT) {
      ...
      Map3D.view.z += Scalar.sgn(pmouseY - mouseY);
    }
  }
}
...
void fileOutput(File selection) {
  if (selection != null) {
    JSON.saveJSONArray(Brain.model.toJSON(), selection.getAbsolutePath());
  }
}

void fileinput(File selection) {
  if (selection != null) {
    Brain.model.fromJSON(JSON.loadJSONArray(selection.getAbsolutePath()));
  }
}
